...
import {app, BrowserWindow, clipboard, dialog, ipcMain, powerMonitor, shell, powerSaveBlocker} from 'electron';
...
function createMainWindow(firstRun, extraTask) {
        ...
        try {
            ...
            if (mainWindow && !mainWindow.isDestroyed()) {
                ...
                if (usingFallbackCert || configVersionOutdated) {
                    message.warning = `Proxy service ${isRestart ? 'restarted' : 'started'} ${usingFallbackCert ? `with a fallback CA certificate. ${ASK_TO_RENEW_CA}` : '.'} ${configVersionOutdated ? CONFIG_OUTDATED_MESSAGE : ''}`
                } else {
                    message.message = `Proxy service ${isRestart ? 'restarted' : 'started'} successfully!`
                }
...
ipcMain.on('start-proxy-profile', async (event, profileIndex, toBeDecided) => {
    ...
    activeProfileIndex = -9;
    decided = {};
    await startProfile(profileIndex, toBeDecided);
});
...
ipcMain.handle('save-edited-profile', async (event, globalSettings, newProfileJson, index, asNew) => {
    ...
    if (restartServers && oldActiveProfileIndex === index) {
        decided = {}
        await startProfile(index, decided, true);
    }
    return {success: true};
});
...
ipcMain.handle('open-main-more-options', async () => {
    try {
        ...
        const options = {
            type: 'question',
            buttons: ['Open help window', GENERATE_CERT_BUTTON_NAME, 'Download CA certificate', ...(systemProxyOn !== null ? [`Toggle system proxy ${systemProxyOn ? 'OFF' : 'ON'}`] : []), 'Cancel'],
            defaultId: 0,
            title: 'more options',
            message: 'Please choose an option:',
        };
