...
export async function checkForUpdates(ignoreSnooze = false) {
    ...
    const updaterState = store.get('updaterState');
    ...
    store.set('updaterState.lastChecked', new Date().toISOString());
...
export function skipVersion(version) {
    ...
    if (!updaterState.skippedVersions.includes(version)) {
        ...
        store.set('updaterState', updaterState);
...
export function snoozeUpdates(days = 4) {
    ...
    store.set('updaterState.snoozedUntil', snoozedUntil.toISOString());
